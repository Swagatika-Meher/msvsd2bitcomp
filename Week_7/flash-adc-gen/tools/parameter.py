...
parser = argparse.ArgumentParser(description="3 bit flash_adc")
parser.add_argument(
    "--specfile",
    required=True,
...
if jsonSpec["generator"] != "flash-adc-gen":
    print('Error: Generator specification must be "flash-adc-gen".')
    sys.exit(1)
...
mFilePublic1 = genDir + args.platform + ".model_flash_adc"
...
if (Fmax > 50) or (Fmin < 5):
    print(
        "Error: Supported frequency must be inside the following range [5 to 50] MHz"
    )
    sys.exit(1)
if Fmax < Fmin:
    print(
...
# get the closest new range of freq max
